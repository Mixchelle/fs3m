...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        json_field = (
            "map" if "map" in sm_fields
            else "mapping" if "mapping" in sm_fields
            else "mapa" if "mapa" in sm_fields
            else None
        )
        rules_field = (
            "rules" if "rules" in sm_fields
            else "regras" if "regras" in sm_fields
            else None
        )
        if not json_field:
            raise RuntimeError(
