...
class Command(BaseCommand):
    help = "Seed NIST CSF 2.0 (PT-BR): domains, controls, questions e template."
    ...
    def handle(self, *args, **options):
        ...
        # Descobre dinamicamente os nomes dos campos JSON/texto na sua model
        ...
        json_field = (
            ...
            if "map" in sm_fields
                ...
                "mapping"
                if "mapping" in sm_fields
                else "mapa" if "mapa" in sm_fields else None
            )
        )
        rules_field = (
            "rules"
            if "rules" in sm_fields
            else "regras" if "regras" in sm_fields else None
        )
        if not json_field:
            raise RuntimeError(
                "ScoringModel: não encontrei o campo JSON de pontuação (esperado: map / mapping / mapa)."
            )
        if not rules_field:
            raise RuntimeError(
                "ScoringModel: não encontrei o campo de regras (esperado: rules / regras)."
            )
        ...
        scoring, _ = ScoringModel.objects.get_or_create(
            slug="maturity-1-5", defaults=defaults
        )
        ...
        for order, (code, title) in enumerate(domain_defs.items()):
            ...
            # garante atualização se já existir
        ...
        controls_pt = {
            ...
            "ID": [
                ...
                (
                    "ID.AM-05",
                    "Ativos são priorizados com base na classificação, criticidade, recursos e urgência na missão?",
                ),
                (
                    "ID.AM-07",
            ...
            "PR": [
                ...
                (
                    "PR.AT-01",
                    "O pessoal recebe conscientização e treinamento... riscos de cibersegurança?",
                ),
                (
                    "PR.AT-02",
                    "Indivíduos em funções especializadas recebem conscientização e treinamento... riscos de cibersegurança?",
                ),
                (
                    "PR.DS-01",
                ...
                (
                    "PR.PS-06",
                    "Práticas seguras de desenvolvimento de software são integradas e monitoradas ao longo do ciclo de vida?",
                ),
                (
                    "PR.IR-01",
            ...
            "RS": [
                ...
                (
                    "RS.AN-03",
                    "Análises são realizadas para estabelecer o que ocorreu e a causa raiz do incidente?",
                ),
                (
                    "RS.AN-06",
            ...
            "RC": [
                ...
                (
                    "RC.RP-04",
                    "Funções críticas de missão e gestão de riscos são consideradas para estabelecer normas operacionais pós-incidente?",
                ),
                (
                    "RC.RP-05",
                ...
                (
                    "RC.CO-03",
                    "Atividades de recuperação e progresso na restauração das capacidades operacionais são comunicados a partes interessadas designadas?",
                ),
                (
                    "RC.CO-04",
        ...
        for dom_code, items in controls_pt.items():
            ...
            for idx, (code, text) in enumerate(items):
                ...
                # garante domínio/ordem atualizados
                ...
                # Question 1: escala (1–5) + scoring model
                ...
                # opções simbólicas (útil para UI)
                ChoiceOption.objects.get_or_create(
                    question=q1,
                    value="1",
                    defaults={"label": "Inicial", "weight": 1, "order": 1},
                )
                ChoiceOption.objects.get_or_create(
                    question=q1,
                    value="2",
                    defaults={"label": "Repetido", "weight": 2, "order": 2},
                )
                ChoiceOption.objects.get_or_create(
                    question=q1,
                    value="3",
                    defaults={"label": "Definido", "weight": 3, "order": 3},
                )
                ChoiceOption.objects.get_or_create(
                    question=q1,
                    value="4",
                    defaults={"label": "Gerenciado", "weight": 4, "order": 4},
                )
                ChoiceOption.objects.get_or_create(
                    question=q1,
                    value="5",
                    defaults={"label": "Otimizado", "weight": 5, "order": 5},
                )

                # Question 2: evidências
                Question.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        "order": 1,
                        "meta": {
                            "placeholder": "Descreva evidências, links, procedimentos, etc."
                        },
                    },
                )
        ...
        for dom_code in ["GV", "ID", "PR", "DE", "RS", "RC"]:
            domain = domains[dom_code]
            qs = Control.objects.filter(framework=framework, domain=domain).order_by(
                "order", "code"
            )
            for control in qs:
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"OK! Framework='{framework.name}', domínios={len(domains)}, "
                f"controles={created_controls}, template='{template.name}'."
            )
        )
